...
* You can now view and modify the weight on a slide using `weight()` and `weight()<-` and also set the `weight` and `filter` to be the same on each slide of a deck. (#440)
